...
/**
 * Backup & Security Features - Acceptance Tests
 * Tests the database backup and security logging functionality
 */

const BASE_URL = 'http://localhost:5000';
const ADMIN_CREDENTIALS = { username: 'cynthia', password: '123456789' };
...
class BackupSecurityTestRunner {
  ...
  async testSecurityLogs() {
    ...
    const loginEvents = data.logs.filter(log => 
  ...
  async testFailedLoginLogging() {
    ...
    const recentFailedLogins = data.logs.filter(log => {
  ...
  async testBackupFileGeneration() {
    console.log('\n📄 Test: Backup file is generated and available');
    
    // Create a schema-only backup (faster than full backup)
    const createResponse = await this.makeAuthenticatedRequest('/api/admin/backups/create', {
      method: 'POST',
      body: JSON.stringify({
        type: 'schema_only',
        description: 'File generation test'
      })
    });
    ...
    const backupId = createData.backupId;

    // Wait for backup to complete (check status periodically)
    let attempts = 0;
    const maxAttempts = 30; // 30 seconds timeout
    let backupCompleted = false;

    while (attempts < maxAttempts && !backupCompleted) {
      await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second
      
      const listResponse = await this.makeAuthenticatedRequest('/api/admin/backups');
      const listData = await listResponse.json();
      const backup = listData.backups.find(b => b.id === backupId);
      
      if (backup) {
        if (backup.status === 'completed') {
          backupCompleted = true;
          console.log(`✅ Backup completed successfully`);
          console.log(`   File size: ${this.formatBytes(backup.size)}`);
          break;
        } else if (backup.status === 'failed') {
          throw new Error(`Backup failed: ${backup.error_message}`);
        }
      }
    ...
    if (!backupCompleted) {
      console.log('⚠️ Backup did not complete within timeout (this may be normal for large databases)');
      this.testResults.push({ test: 'Backup File Generation', status: 'TIMEOUT' });
    } else {
      this.testResults.push({ test: 'Backup File Generation', status: 'PASSED' });
    }
  }
  ...
  printResults() {
    ...
    if (passed === total) {
      ...
      console.log('⚠️ Some tests failed or timed out. Please check the implementation.');
    }
  }

  async runAllTests() {
    try {
      console.log('🚀 Starting Backup & Security Acceptance Tests...');
      ...
      await this.testBackupFileGeneration();
      
      this.printResults();
      
    } catch (error) {
      console.error('\n❌ Test failed:', error.message);
      this.testResults.push({ test: 'Test Execution', status: 'FAILED' });
      this.printResults();
      process.exit(1);
    } finally {
      await this.logout();
