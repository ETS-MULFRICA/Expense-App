...
async function testRBACIntegration() {
  console.log('🧪 Testing RBAC Integration    console.log('Cleaning up test data...');
    await storage.removeRoleFromUser(testUserId, editorRole.id);
    await storage.deleteRole(editorRole.id);
    console.log('✓ Test role and assignments cleaned up (keeping existing users)');d');
  console.log('================================================\n');
  ...
  try {
    ...
    // Step 1: Create "Editor" role with limited permissions
    console.log('1.1 Creating "Editor" role...');
    ...
    // Step 2.2: Assign editor role to user
    console.log('2.2 Assigning editor role to user...');
    await storage.assignRoleToUser(testUserId, editorRole.id);
    console.log('✓ Editor role assigned to user successfully');
    ...
    try {
      ...
      await storage.deleteUser?.(adminUserId);
      console.log('✓ Test users deleted');
    } catch (error) {
      console.log('✓ Test role cleaned up (users may persist for other tests)');
    }
